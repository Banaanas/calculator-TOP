...
const checkDecimal = () => {
  ...
  while (n < calculatorObj.calculationArray.length) {
    ...
    for (let index = 0; index < calculatorObj.calculationArray.length; index++) {
      if (calculatorObj.calculationArray[index] !== "+" && calculatorObj.calculationArray[index + 1] !== "+"
                && calculatorObj.calculationArray[index] !== "-" && calculatorObj.calculationArray[index + 1] !== "-"
                && calculatorObj.calculationArray[index] !== "*" && calculatorObj.calculationArray[index + 1] !== "*"
                && calculatorObj.calculationArray[index] !== "/" && calculatorObj.calculationArray[index + 1] !== "/"
                && calculatorObj.calculationArray[index + 1] !== undefined) {
        // eslint-disable-next-line max-len
        const product = calculatorObj.calculationArray[index].concat(calculatorObj.calculationArray[index + 1]);
